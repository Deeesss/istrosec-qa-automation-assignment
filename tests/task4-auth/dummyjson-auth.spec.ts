...
test.describe('Task 4B - DummyJSON API authentication and authorization', () => {
  ...
  // Why: Integrations of a security product authenticate with tokens; both must be issued and must differ, so the access token sent with every request cannot also renew the session.
  ...
  // Why: Operator usernames are often predictable, so a login request without a password must be rejected instead of authenticating by username alone.
  ...
  // Why: A failed login must not leak tokens in its response, because one leaked token would give an attacker API access to endpoint and user data.
  ...
  // Why: Permissions and audit logs depend on the token resolving to the user who logged in; the fresh request context proves that the token alone grants access.
  ...
  // Why: Without this check, anyone who can reach the API could read user and endpoint data, so a request without a token must be rejected with 401.
  ...
  // Why: Attackers replay forged, tampered or old stolen tokens, so the API must validate the token itself, not only the presence of an Authorization header.
